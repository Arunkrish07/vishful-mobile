...
const CONFIDENCE_COLORS = {
  ...
  fallback: { bg: '#EFF6FF', text: '#2563EB', border: '#BFDBFE' },
};
...
export default function TenantHomeScreen({ navigation }: any) {
    ...
    return (
      <View style={{ alignItems: 'center', paddingVertical: 28, gap: 20 }}>
        ...
        {!recAvailable && voiceStep === 'idle' && (
          <View style={{ backgroundColor: '#EEF3FF', borderRadius: 12, padding: 12, width: '100%', borderWidth: 1, borderColor: '#BFDBFE', flexDirection: 'row', alignItems: 'flex-start', gap: 10 }}>
            <Ionicons name="information-circle-outline" size={18} color="#1D4ED8" style={{ marginTop: 1 }} />
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: 13, fontWeight: '700', color: '#1D4ED8', marginBottom: 2 }}>Voice not available on this browser</Text>
              <Text style={{ fontSize: 12, color: '#1D4ED8', lineHeight: 18 }}>Tap the button below to type your complaint instead.</Text>
            </View>
          </View>
        )}
        ...
        <View style={{ width: 100, height: 100, alignItems: 'center', justifyContent: 'center' }}>
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              backgroundColor: voiceStep === 'recording' ? '#DC2626' : '#1D4ED8',
              alignItems: 'center', justifyContent: 'center',
              shadowColor: voiceStep === 'recording' ? '#DC2626' : '#1D4ED8',
        ...
        {voiceStep === 'idle' && (
          <View style={{ width: '100%', gap: 6 }}>
            <Text style={{ fontSize: 11, fontWeight: '700', color: '#94A3B8', letterSpacing: 0.8, marginBottom: 2 }}>
      ...
      <View style={{ gap: 14 }}>
        {selectedType && (
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, backgroundColor: confColors.bg, borderRadius: 12, padding: 12, borderWidth: 1, borderColor: confColors.border }}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={{ fontSize: 15, fontWeight: '700', color: '#0F172A', marginTop: 1 }}>{selectedType.name}</Text>
        ...
        <View style={{ gap: 6 }}>
          ...
          {!canRecord() && !editedTranscript && (
            <Text style={{ fontSize: 12, color: '#94A3B8', marginTop: -2, marginBottom: 4 }}>
          ...
          <TextInput
            ...
            style={{ backgroundColor: '#fff', borderRadius: 12, padding: 12, fontSize: 14, color: '#0F172A', borderWidth: 1.5, borderColor: editedTranscript ? '#E5E7EB' : '#C7D2FE', minHeight: 90, textAlignVertical: 'top', lineHeight: 22 }}
          />
        </View>
          ...
          {loadingIssueTypes
            ...
            : issueTypes.map(it => (
              ...
              >
                ...
                <Text style={{ fontSize: 14, flex: 1, fontWeight: selectedTypeId === it.id ? '700' : '400', color: selectedTypeId === it.id ? '#0F172A' : '#64748B' }}>{it.name}</Text>
                {it.priority && <Text style={{ fontSize: 11, color: '#94A3B8' }}>{it.priority}</Text>}
              </TouchableOpacity>
            ))
          }
        </View>
        ...
        <TouchableOpacity onPress={resetVoice} style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 5, paddingVertical: 4 }}>
          ...
          <Text style={{ fontSize: 13, color: '#1D4ED8', fontWeight: '600' }}>Record Again</Text>
        </TouchableOpacity>
      </View>
    );
  };
  ...
  const renderVoiceSuccess = () => (
    <View style={{ alignItems: 'center', paddingVertical: 32, gap: 16 }}>
      ...
      <Text style={{ fontSize: 13, color: '#64748B', textAlign: 'center', lineHeight: 20, paddingHorizontal: 12 }}>
        Your {classification?.issueTypeName || 'maintenance'} request has been logged. Our team will respond shortly.
      </Text>
      <TouchableOpacity onPress={closeVoiceModal} style={{ backgroundColor: '#1D4ED8', borderRadius: 12, paddingHorizontal: 36, paddingVertical: 14, marginTop: 4 }}>
  ...
  const renderVoiceError = () => (
    <View style={{ alignItems: 'center', paddingVertical: 28, gap: 14 }}>
      ...
      <Text style={{ fontSize: 17, fontWeight: '800', color: '#0F172A' }}>Something Went Wrong</Text>
      ...
      <View style={{ flexDirection: 'row', gap: 12, marginTop: 4 }}>
        <TouchableOpacity onPress={() => { setVoiceError(''); setVoiceStep('idle'); }} style={{ backgroundColor: '#1D4ED8', borderRadius: 12, paddingHorizontal: 24, paddingVertical: 12 }}>
          <Text style={{ color: '#fff', fontWeight: '800', fontSize: 14 }}>Try Again</Text>
        </TouchableOpacity>
        <TouchableOpacity onPress={closeVoiceModal} style={{ backgroundColor: '#F8FAFC', borderRadius: 12, paddingHorizontal: 24, paddingVertical: 12, borderWidth: 1, borderColor: '#E5E7EB' }}>
    ...
    <GlassBackground>
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        ...
        <View style={[glass.header, { flexDirection: 'row', alignItems: 'center', paddingHorizontal: spacing.xl, paddingVertical: spacing.lg }]}>
          <View style={{ flex: 1 }}>
            <Text style={{ fontSize: fontSize.xs, color: '#94A3B8', fontWeight: '600', letterSpacing: 0.5 }}>WELCOME</Text>
            <Text style={{ fontSize: fontSize.lg, fontWeight: '800', color: '#0F172A', letterSpacing: -0.3 }}>{user?.userName || 'Tenant'}</Text>
          </View>
          ...
          <TouchableOpacity
            ...
            style={{
              ...
              backgroundColor: '#1D4ED8', paddingHorizontal: 14,
        ...
        >
          ...
          {accountNotLinked && !detailsLoading && (
            <View style={{ backgroundColor: '#FFEDD5', borderRadius: borderRadius.lg, padding: spacing.lg, borderWidth: 1.5, borderColor: '#EA580C', flexDirection: 'row', alignItems: 'flex-start', gap: 12 }}>
              <View style={{ width: 40, height: 40, borderRadius: 20, backgroundColor: '#FFF7ED', borderWidth: 1.5, borderColor: '#EA580C', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                <Ionicons name="warning-outline" size={20} color="#EA580C" />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ fontSize: fontSize.sm, fontWeight: '800', color: '#EA580C', marginBottom: 4 }}>Account Not Linked</Text>
          ...
          <TouchableOpacity
            onPress={() => { resetVoice(); setVoiceModalOpen(true); }}
            style={{ backgroundColor: '#FFFFFF', borderRadius: 16, padding: spacing.lg, borderWidth: 1, borderColor: '#EEF1F6', shadowColor: '#0F172A', shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, flexDirection: 'row', alignItems: 'center', gap: 14 }}
          >
            <View style={{ width: 48, height: 48, borderRadius: 24, backgroundColor: '#1D4ED8', alignItems: 'center', justifyContent: 'center' }}>
              <Ionicons name="mic" size={24} color="#fff" />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={{ fontSize: fontSize.md, fontWeight: '800', color: '#0F172A' }}>Raise a Complaint</Text>
              <Text style={{ fontSize: fontSize.xs, color: '#1D4ED8', marginTop: 2 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 12 }}>
            {/* Total Due */}
            <View style={{ flex: 1, backgroundColor: '#FFFFFF', borderRadius: 16, padding: spacing.lg, borderWidth: 1, borderColor: '#EEF1F6', shadowColor: '#0F172A', shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 } }}>
              <View style={{ width: 30, height: 30, borderRadius: 10, backgroundColor: totalDue > 0 ? '#FEE2E2' : '#DCFCE7', alignItems: 'center', justifyContent: 'center', marginBottom: 8 }}>
                <Ionicons name="wallet-outline" size={15} color={totalDue > 0 ? '#DC2626' : '#16A34A'} />
              </View>
              <Text style={{ fontSize: 10, fontWeight: '800', color: '#94A3B8', letterSpacing: 0.8 }}>TOTAL DUE</Text>
              <Text style={{ fontSize: fontSize.lg, fontWeight: '800', color: totalDue > 0 ? '#DC2626' : '#16A34A', marginTop: 2 }} numberOfLines={1}>
            ...
            {/* Open Tickets */}
            <View style={{ flex: 1, backgroundColor: '#FFFFFF', borderRadius: 16, padding: spacing.lg, borderWidth: 1, borderColor: '#EEF1F6', shadowColor: '#0F172A', shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 } }}>
              <View style={{ width: 30, height: 30, borderRadius: 10, backgroundColor: '#EEF3FF', alignItems: 'center', justifyContent: 'center', marginBottom: 8 }}>
                <Ionicons name="construct-outline" size={15} color="#1D4ED8" />
              </View>
              <Text style={{ fontSize: 10, fontWeight: '800', color: '#94A3B8', letterSpacing: 0.8 }}>OPEN TICKETS</Text>
              <Text style={{ fontSize: fontSize.lg, fontWeight: '800', color: '#0F172A', marginTop: 2 }}>{openTicketCount}</Text>
              <Text style={{ fontSize: fontSize.xs, color: '#94A3B8', marginTop: 2 }}>
          ...
          {detailsLoading && !tenantDetails ? (
            <View style={{ backgroundColor: '#FFFFFF', borderRadius: 16, padding: spacing.lg, alignItems: 'center', gap: 8, borderWidth: 1, borderColor: '#EEF1F6', shadowColor: '#0F172A', shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 } }}>
              <ActivityIndicator size="small" color="#1D4ED8" />
              <Text style={{ fontSize: fontSize.xs, color: '#94A3B8' }}>Loading your details…</Text>
            </View>
          ) : tenantDetails?.tenant ? (
            <View style={{ backgroundColor: '#FFFFFF', borderRadius: 16, borderWidth: 1, borderColor: '#EEF1F6', shadowColor: '#0F172A', shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, overflow: 'hidden' }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', padding: spacing.lg, gap: 14, borderBottomWidth: 1, borderBottomColor: '#DCE7FF', backgroundColor: '#EEF3FF' }}>
                ...
                <View style={{ flex: 1 }}>
                  <Text style={{ fontSize: fontSize.md, fontWeight: '800', color: '#0F172A' }}>{tenantDetails.tenant.full_name || user?.userName}</Text>
                  <Text style={{ fontSize: fontSize.xs, color: '#64748B', marginTop: 2 }}>{tenantDetails.tenant.phone}</Text>
          ...
          {(tenantDetails?.allotment || tenantLocation) && (
            <View style={{ backgroundColor: '#FFFFFF', borderRadius: 16, padding: spacing.lg, borderWidth: 1, borderColor: '#EEF1F6', shadowColor: '#0F172A', shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 } }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: spacing.sm }}>
                <Ionicons name="home-outline" size={14} color="#1D4ED8" />
                <Text style={{ fontSize: 10, fontWeight: '800', color: '#94A3B8', letterSpacing: 1 }}>ACCOMMODATION</Text>
              </View>
              {[
                ...
                <View key={row.label} style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 6 }}>
                  <Text style={{ fontSize: fontSize.sm, color: '#64748B' }}>{row.label}</Text>
                  <Text style={{ fontSize: fontSize.sm, fontWeight: '700', color: '#0F172A', flex: 1, textAlign: 'right', marginLeft: 16 }} numberOfLines={1}>{row.value}</Text>
                </View>
              ))}
            </View>
          )}
          ...
          {announcements.length > 0 && (
            <View style={{ gap: 8 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                ...
                <Text style={{ fontSize: 10, fontWeight: '800', color: '#94A3B8', letterSpacing: 1 }}>ANNOUNCEMENTS</Text>
              </View>
              {announcements.map((a: any) => {
                ...
                return (
                  <TouchableOpacity
                    ...
                    style={{ backgroundColor: '#FFFFFF', borderRadius: 16, padding: spacing.lg, borderWidth: 1, borderColor: '#EEF1F6', shadowColor: '#0F172A', shadowOpacity: 0.05, shadowRadius: 10, shadowOffset: { width: 0, height: 4 }, gap: 4 }}
                  >
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                      <Text style={{ flex: 1, fontSize: fontSize.sm, fontWeight: '800', color: '#0F172A' }} numberOfLines={expanded ? undefined : 1}>
                        {a.title || 'Announcement'}
                      </Text>
                      <Ionicons name={expanded ? 'chevron-up' : 'chevron-down'} size={16} color="#94A3B8" />
                    </View>
                    {when ? (
                      <Text style={{ fontSize: fontSize.xs, color: '#94A3B8' }}>{formatDate(when, '')}</Text>
                    ) : null}
                    {a.content ? (
                      <Text style={{ fontSize: fontSize.sm, color: '#64748B', marginTop: 2, lineHeight: 20 }} numberOfLines={expanded ? undefined : 2}>
                        {a.content}
                      </Text>
                    ) : null}
                  </TouchableOpacity>
          ...
          <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }} edges={['top', 'bottom']}>
            <View style={{ flexDirection: 'row', alignItems: 'center', paddingHorizontal: 20, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: '#E5E7EB', backgroundColor: '#fff' }}>
              ...
              <View style={{ flex: 1, flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                ...
                <Text style={{ fontSize: 17, fontWeight: '800', color: '#0F172A' }}>{voiceModalTitle}</Text>
              </View>
            </View>
            ...
            <ScrollView contentContainerStyle={{ padding: 20, paddingBottom: 32 }} keyboardShouldPersistTaps="handled">
              ...
              {voiceStep === 'submitting' && (
                <View style={{ alignItems: 'center', paddingVertical: 48, gap: 16 }}>
                  ...
                  <Text style={{ fontSize: 16, fontWeight: '700', color: '#0F172A' }}>Creating your ticket…</Text>
                </View>
              )}
            ...
            {voiceStep === 'review' && (
              <View style={{ padding: 20, paddingBottom: Platform.OS === 'ios' ? 8 : 20, borderTopWidth: 1, borderTopColor: '#E5E7EB', backgroundColor: '#fff' }}>
                <TouchableOpacity
                  ...
                  style={{ backgroundColor: canSubmit ? '#1D4ED8' : '#94A3B8', borderRadius: 12, paddingVertical: 16, alignItems: 'center', flexDirection: 'row', justifyContent: 'center', gap: 8 }}
                >
        ...
        <Modal visible={!!approvalTicket} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => setApprovalTicket(null)}>
          <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }} edges={['top', 'bottom']}>
            <View style={{ flexDirection: 'row', alignItems: 'center', padding: spacing.xl, borderBottomWidth: 1, borderBottomColor: '#E5E7EB' }}>
              ...
              <Text style={{ fontSize: fontSize.lg, fontWeight: '800', color: '#0F172A' }}>Review Completed Work</Text>
            </View>
              ...
              {approvalTicket && (
                <>
                  <View style={{ backgroundColor: '#FFEDD5', borderRadius: borderRadius.lg, padding: spacing.md, borderWidth: 1, borderColor: '#EA580C' }}>
                    ...
                    {approvalTicket.description && <Text style={{ fontSize: fontSize.sm, color: '#64748B', marginTop: 4 }}>{approvalTicket.description}</Text>}
                  </View>
                  ...
                  {showRejectInput && (
                    <TextInput
                      style={{ backgroundColor: '#fff', borderRadius: borderRadius.md, padding: spacing.md, fontSize: fontSize.md, color: '#0F172A', borderWidth: 1, borderColor: '#DC2626', minHeight: 80, textAlignVertical: 'top' }}
        ...
        <Modal visible={noticeModalOpen} animationType="slide" presentationStyle="pageSheet" onRequestClose={() => { if (!noticeSaving) { setNoticeModalOpen(false); setNoticeExitDate(''); setNoticeNotes(''); } }}>
          <SafeAreaView style={{ flex: 1, backgroundColor: '#FFFDF5' }} edges={['top', 'bottom']}>
            <View style={{ flexDirection: 'row', alignItems: 'center', padding: spacing.xl, borderBottomWidth: 1, borderBottomColor: '#F0E4B0' }}>
              ...
              <Text style={{ fontSize: fontSize.lg, fontWeight: '800', color: '#0F172A', flex: 1 }}>Record Move-Out Notice</Text>
            </View>
            <ScrollView contentContainerStyle={{ padding: spacing.xl, gap: 20 }}>
              <View style={{ gap: 8 }}>
                <Text style={{ fontSize: fontSize.sm, fontWeight: '700', color: '#0F172A' }}>Expected Move-Out Date <Text style={{ color: '#DC2626' }}>*</Text></Text>
                <DateField value={noticeExitDate} onChange={setNoticeExitDate} placeholder="Select move-out date" />
              </View>
              <View style={{ gap: 8 }}>
                <Text style={{ fontSize: fontSize.sm, fontWeight: '700', color: '#0F172A' }}>Additional Notes <Text style={{ color: '#94A3B8', fontWeight: '400' }}>(optional)</Text></Text>
                <TextInput value={noticeNotes} onChangeText={setNoticeNotes} placeholder="Any specific reason or request…" placeholderTextColor="#94A3B8" multiline numberOfLines={4} style={{ backgroundColor: '#fff', borderRadius: borderRadius.md, padding: spacing.md, fontSize: fontSize.md, color: '#0F172A', borderWidth: 1.5, borderColor: '#E5E7EB', minHeight: 100, textAlignVertical: 'top' }} />
              </View>
